...
%         gammaCmb                    = ft_combineplanar(cfg, gammaFreq_bsl);
        ...
        cfg             = [];
%         cfg.jackknife   = 'yes';
        gammaDesc       = ft_freqdescriptives(cfg, PD_ERFgammaCmb{subj,cond});
        ...
        maxGammaChExtract(subj, cond) = ...
            mean(max(gammaDesc.powspctrm(chanPos,:,find(gammaDesc.time>=0.02 & gammaDesc.time<=0.09)), [],3));
%          mean(mean(max(gammaCmb.powspctrm(:,chanPos,:,find(gammaCmb.time>=0.02 & gammaCmb.time<=0.09)), [],4),2))
%         mean(mean(max(gammaCmb.powspctrm(:,chanPos,:,find(gammaCmb.time>=0.02 & gammaCmb.time<=0.09)), [],4),1))
        
%         allGamma{subj, cond} = gammaDesc;
%         tok = tokenize(char(file_sub(subj, cond)),'-');
%         allGamma{subj,cond}.name  = tok{1};
...
% for subj = 1:10         
    ...
    for cond = 1:7
        ...
        ft_topoplotER(cfg, GA_gamma_Contorl_ind{:,cond})
        title(cond)
    end
    suptitle('C')
...
[AX,H1,H2] = plotyy([time_vector; time_vector+0.1]' , ...
    [mean(gammaPD([1:10],:),1), mean(gammaPD([1:10],:),2)]',...
    [1 2 4 8], mean(gammaPD(:,1)-gammaPD(:,2)));
...
errorbar(time_vector+0.2, mean(gammaPD(subjP,:),1),  sem(gammaPD(subjP,:)), '-r.');
hold on
errorbar(time_vector, mean(gammaControlmean(contS,:),1),  sem(gammaControlmean(contS,:)), '-b.')
ylim([0.9 1.65])
