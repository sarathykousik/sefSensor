...
cfg = [];
for subjLooop = 
    for condLoop  = 1:length(filename)
    %Choose filenames
    [row, col]=find(strcmp(raw, subjName{3})==1);
    % Load data
    workData{loop}     = load(filename(loop).name);
    workData{loop}.filename = filename(loop).name;
    
    % Combineplanar
    cmbData_tlck{loop}     = ft_combineplanar(cfg, workData{loop}.saveData.tlck_avg);
    cmbData_tlck{loop}.filename = filename(loop).name;
    cmbData_TFR{loop}       = ft_combineplanar(cfg, workData{loop}.saveData.TFR_hann);
