...
% set(gca,'Fontname','Helvetica');
% 
% htitle = title('Amplitude of N20m');
% set(htitle,'Fontsize',24);
% set(htitle,'FontWeight','bold');
% set(htitle,'Fontname','Helvetica');
...
% 
%         
%         topoGammaC{subj,cond}.dimord = 'chan_freq';
...
%        GAgammaControl{cond} = ft_freqgrandaverage(cfg, topoGammaC{:,cond});
%         
% end
...
for loop = 1:7
    dat = GAgammaPD{loop};
    dat.powspctrm = GAgammaPD{loop}.powspctrm.*100;
...
% Load: gammaPD, gammaControlmean
% cd('J:\MEG_Research\SEF\sefgammaStatistics\SEFgammaStat\75Hz-25Hz')
% load gammaControlmean.mat; load gammaPD.mat
figure
contS = [1:10];
subjP = [1:10];
time_vector = [1 2 3 4 5 6 8];
...
set(e1, 'MarkerSize', 14,'Color', clrOp1 , 'MarkerFaceColor', clrOp1 , ...
    'MarkerEdgeColor', clrOp1);
set(e1,'LineWidth', 4.5 )

e2=errorbar(time_vector, mean(Controlnorm(contS,:),1),sem(Controlnorm(contS,:)),   '-s')
set(e2, 'MarkerSize', 14,'Color', clrOp2 , 'MarkerFaceColor', clrOp2 , ...
    'MarkerEdgeColor', clrOp2);
set(e2,'LineWidth', 4.5 )
...
%% DBS OFF 120min to MED ON? UPDRS  vs gamma - No
dat_1 = [gammaPD(:,6)-gammaPD(:,7)];
dat_2 = [updrs(:,3)-updrs(:,7)];%diseaseYrs;
scatter(dat_1, dat_2), lsline
% axis([0 1 0 25])
...
%% DBS ON to untreated? UPDRS  vs gamma - YES
dat_1 = [gammaPD(:,6)-gammaPD(:,1)];
dat_2 = [updrs(:,3)-updrs(:,1)];%diseaseYrs;
scatter(dat_1, dat_2), lsline
% axis([0 1 0 25])
...
%% DBS ON to untreated? UPDRS  vs gamma - YES
dat_1 = [gammaPD(:,1)-gammaPD(:,2)]+[gammaPD(:,6)-gammaPD(:,7)];
dat_2 = diseaseYrs;
scatter(dat_1, dat_2), lsline
% axis([0 1 0 25])
