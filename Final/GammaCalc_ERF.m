...
% proc                             = [];
% proc.data_folder                 = 'J:\MEG_Research\SEF\SEFVisClean\09';
% proc.save_folder                 = 'J:\MEG_Research\SEF\SEFGamma3tap\2tap-75Hz-10ms';
...
% filenames      = dir('*.mat');
for subj  = 1:10
    for cond  = 1:7 % length(filenames)
        disp('#######################################')
    %     [a b c]     =   fileparts(filenames(loop).name);
        disp(['   ****    ',tlckPD{subj,cond}.name,'     ****    '])
    %     load(filenames(loop).name)
        ...
        cfg.tapsmofrq    = 25;
        gammaFreq{subj,cond}        = ft_freqanalysis(cfg, tlckPD{subj,cond});
...
%         save([proc.save_folder,'\freq\' , b, '-gammaFreq'], 'gammaFreq', '-v7.3');
        ...
        gammaFreq_bsl{subj,cond}    = ft_freqbaseline(cfg, gammaFreq{subj,cond});
    
        cfg                         = [];
        gammaCmb{subj,cond}         = ft_combineplanar(cfg, gammaFreq_bsl{subj,cond});
